...
void *servo1(void *arg)
{
    ...
    for (int i = 0; i < STEPS; i++)
    {
        ...
        for (int j = current; j != target; j += (j < target ? 1 : -1))
        {
            ...
            {
                int rest = angleToPwm(90);
                for (int k = j; k != rest; k += (k < rest ? 1 : -1))
                {
                    pwmWrite(SERVO1, k);
                    delay(20);
                }
...
void *magnet(void *arg)
{
    ...
    for (int i = 0; i < STEPS; i++)
    {
        ...
        int expected = modes[selectedMode][4][i];

        digitalWrite(ledPin, expected ? HIGH : LOW);

        if (i != 5 && ir_check != expected)
        {
            printf("ERROR: object not detected\n");
            ir_error = 1;
            break;
        }
        pthread_barrier_wait(&barrier);
